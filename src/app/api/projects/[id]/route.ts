...
export const PATCH = withCapability("Create/Edit Projects", async (req, { params }: { params: { id: string } }, user) => {
  ...
  if ("name" in body) {
    data.name = body.name;
    const current = await prisma.project.findUniqueOrThrow({ where: { id: params.id }, select: { programId: true } });
    data.slug = await uniqueSlug(
      slugify(body.name),
      async (candidate) => (await prisma.project.count({ where: { programId: current.programId, slug: candidate, NOT: { id: params.id } } })) > 0
    );
  }
