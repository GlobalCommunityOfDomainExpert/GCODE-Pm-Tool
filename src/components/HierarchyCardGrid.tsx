...
export function HierarchyCardGrid({
  ...
  return (
    <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4">
      {menuOpenId && <div className="fixed inset-0 z-[5]" onClick={() => setMenuOpenId(null)} />}
      {items.map((item) => (
        <div
          key={item.id}
        ...
        >
          <button
            ...
            className="absolute right-3 top-3 z-10 flex h-7 w-7 items-center justify-center rounded text-text-secondary hover:bg-slate-100"
          ...
          {menuOpenId === item.id && (
            <div
              className="absolute right-3 top-11 z-20 w-36 overflow-hidden rounded-md border border-border bg-white py-1 shadow-xl"
